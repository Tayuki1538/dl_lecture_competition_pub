...
class Patching(nn.Module):
    def __init__(self, patch_height, patch_width):
        ...
        """
        super().__init__()
        self.net = Rearrange("(b c) (h ph) (w pw) -> b (h w) (ph pw c)", c = 1, ph = patch_height, pw = patch_width)
...
class LinearProjection(nn.Module):
    def __init__(self, patch_dim, dim):
        """ [input]
            - patch_dim (int) : 一枚あたりのパッチのベクトルの長さ（= channels * patch_height * patch_width）
...
class ViT(nn.Module):
    def __init__(self, image_height, image_width, patch_height, patch_width, n_classes, dim, depth, n_heads, channels = 1, mlp_dim = 2048):
        """ [input]
